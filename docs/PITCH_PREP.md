# SplitRails — Hackathon Pitch Prep & Q&A Guide

Use this guide to prepare for your 3-minute pitch presentation. You can copy this content directly into Google Docs or print it to PDF from your editor.
...
|---|---|---|
| **Frontend UI** | React (TS) + Vite | Fast Single Page Application (SPA), hosted on Vercel. |
| **Styling** | TailwindCSS (v4) | Premium glassmorphism UI, custom gradients, and micro-animations. |
| **Biometrics** | WebAuthn API | Triggers Face ID, Touch ID, or Windows Hello in-browser. |
| **Biometric Crypto** | Web Crypto API | Client-side ECDSA P-256 signature verification over transaction hashes. |
| **Stellar integration** | @stellar/stellar-sdk | Connects to Horizon & Soroban RPC nodes to read on-chain state. |
| **Wallet connection** | @stellar/freighter-api | Connects Freighter browser extension to securely sign transactions. |
| **Smart Contracts** | Soroban / Rust | ESCROW contract deployed to Stellar Testnet (`CDENUPG5...`). |
| **Accounting Integration** | Custom GL Exporter | Outputs structured CSV files with custom Cost Centers and Project IDs. |

---

## 🚀 The User Journey (Simplified)

1. **Setup & Split:** The expense organizer logs in, inputs total cost, splits shares, and inputs cost-center accounting tags.
2. **On-Chain Initialization:** The organizer deploys a brand new instance of the escrow contract from the on-chain WASM hash and initializes it with the vendor address, deadline, and shares.
3. **Biometric Gate:** Each participant clicks "Pay" on their link, triggering a biometric pre-flight check to sign a transaction-bound challenge.
4. **On-Chain Settlement:** Once biometrics pass, Freighter submits the USDC payment to the contract.
5. **Atomic Release:** Once all participants have cleared, the contract atomically releases the full lump sum to the vendor's wallet.
6. **Accounting Audit:** The organizer views the live timeline and exports a GL-tagged CSV to sync with QuickBooks or Xero.
